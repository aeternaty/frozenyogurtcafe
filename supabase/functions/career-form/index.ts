...
serve(async (req) => {
  ...
  try {
    ...
    const applicantAge = parseInt(formData.get('applicant-age')?.toString() || '0')
    ...
    if (!applicantName || !applicantAge || !applicantEmail || !applicantPhone || 
        !preferredLocation || !positionType || !availability || !whyJoin || !terms) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields. Please fill out all required information.' }), 
        { 
          status: 400, 
    ...
    if (!validLocations.includes(preferredLocation) || 
        !validPositionTypes.includes(positionType) ||
        !validAvailability.includes(availability)) {
      return new Response(
        JSON.stringify({ error: 'Invalid selection for location, position type, or availability' }), 
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    if (existingApp) {
      return new Response(
        JSON.stringify({ 
          message: 'Thank you for your interest! We have already received your application and will review it within 3-5 business days.' 
        }), 
        { 
          status: 200, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
    ...
    if (inappropriatePattern.test(whyJoin) || inappropriatePattern.test(applicantName)) {
      return new Response(
        JSON.stringify({ 
          message: 'Thank you for your application! We will review it and contact you within 3-5 business days.' 
        }), 
        { 
          status: 200, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
